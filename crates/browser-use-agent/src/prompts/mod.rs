...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "When the connection is blocked, READ THE CANDIDATE FIELDS — specifically `browser_running` and `remote_debugging_enabled`. IGNORE the `state` string for routing; the field values are the source of truth. Each fix changes Chrome's state, so re-run `browser connect local` to see what's next. Problems chain (Chrome not running → launch → retry → may now be fully connectable, OR may now need permission setup → handle whichever comes next).\n\n",
            ...
            "  C) `browser_running: true` AND `remote_debugging_enabled: true` BUT the connect attempt itself returns `state: \"permission-blocked\"` or HTTP 403 / Forbidden in the error: the on-disk toggle is already on; Chrome is showing (or has queued) its per-session \"Allow remote debugging?\" popup and waiting on the user. DO NOT touch chrome://inspect, DO NOT tell the user to tick the checkbox — it's already ticked, that'd be confusing. Instead:\n",
            "      1. Tell the user: \"Chrome is showing an 'Allow remote debugging?' popup somewhere — check each open Chrome window (multi-profile setups put it in only one) and click Allow. Reply when done.\" Mention that with multiple profile windows the popup often appears in a window they aren't looking at.\n",
