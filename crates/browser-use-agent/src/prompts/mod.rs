...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "      2. If a default profile is already set, use the `shell` tool to open Chrome — DO NOT ask the user to do this manually. macOS: `open -a \"Google Chrome\"`. Linux: `google-chrome &` (or whichever Chromium binary is on PATH — `chromium`, `brave-browser`, `microsoft-edge`). Windows: `cmd /c start chrome`. Only fall back to asking the user if the shell command errors AND you can't recover. Give Chrome a moment to start (~1–2s), then retry `browser connect local`.\n",
            ...
            "      1. If the blocked candidate identifies the profile/window, focus that path. Otherwise use the default profile setting. Only open a profile window if you need to help the user find the popup.\n",
