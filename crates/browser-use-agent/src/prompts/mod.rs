...
pub fn browser_mode_instruction(mode: &str) -> String {
    ...
    match normalized.as_str() {
        "local" | "local-chrome" => concat!(
            ...
            "For tasks that depend on an existing login, account, cookies, or a private web app/domain where the user's existing session matters, choose the local Chrome profile before the first connect. Run `browser profile suggest --domain <domain>` when the domain is known, otherwise run `browser local profiles`. If more than one plausible profile is returned, ask the user which profile/window to use, wait for their answer, keep that choice in chat history only, make sure that profile's Chrome window is open or targeted when Chrome is not already running in that profile, then run `browser connect local`. For public/no-login tasks, connect directly.\n\n",
            ...
            "      1. If more than one local profile/window could receive the chrome://inspect page, run `browser local profiles`, ask which profile to use, and keep that choice in chat history only. If the browser/profile is obvious from the candidate or only one profile is available, proceed silently.\n",
            ...
            "      1. If the blocked candidate identifies the profile/window, focus that path. Otherwise, if multiple profile windows are plausible, run `browser local profiles`, ask which profile the user wants to use, and keep that choice in chat history only. Only open a profile window if you need to help the user find the popup.\n",
            ...
            "  1. Keep the profile choice in chat history only; do not persist it in settings. Do not open an empty Chrome window just because a profile was selected.\n",
