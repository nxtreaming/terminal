//! Event/message rollback and turn-boundary helpers extracted from `lib.rs` (Phase 0.1 carve).
//!
...
use crate::constants::*;
use crate::{
    is_skill_context_message, is_subagent_notification_context_message, is_turn_aborted_message,
    provider_message_is_inter_agent_instruction,
};
...
pub(crate) fn rollback_filtered_events(events: &[EventRecord]) -> Vec<&EventRecord> {
    let mut messages = Vec::new();
    rollback_filtered_events_after(events, 0, &mut messages)
}

pub(crate) fn rollback_filtered_events_after<'a>(
...
pub(crate) fn rollback_filtered_events_after_for_fork<'a>(
    events: &'a [EventRecord],
    after_seq: i64,
    messages: &mut Vec<Value>,
) -> Vec<&'a EventRecord> {
...
pub(crate) fn is_real_user_event(event: &EventRecord) -> bool {
    matches!(
...
pub(crate) fn provider_history_has_open_turn(messages: &[Value]) -> bool {
    let Some(user_pos) = messages
        .iter()
