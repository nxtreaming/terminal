...
### ▶ S-rollout — IN PROGRESS
- [ ] WP-S-rollout — rollout hardening (archival/fork/truncation over SQLite write-sink). Legacy `browser-use-core/src/rollback.rs` + `persistence.rs`; reuse the agent crate's existing `session/*` seam. NEW module `rollout/*` + pub mod line. Network-free tests (real Store on tempdir).
