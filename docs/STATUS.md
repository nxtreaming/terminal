...
### ▶ S-skills — IN PROGRESS
- [ ] WP-S-skills-plugins — skill/plugin discovery (roots + precedence), ~2%-of-context budgeted `<skills_instructions>` injection, `$`/`@`/`skill://` mentions. Codex `core/src/skills.rs` + legacy browser-use-core skills/plugins. NEW module `skills/*` + pub mod line. Network-free tests (tempdir fixtures).
