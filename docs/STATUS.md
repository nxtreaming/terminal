...
### ▶ S-hooks — IN PROGRESS
- [ ] WP-S-hooks — hook runtime (matcher groups, command hooks) + PermissionRequest event + Prompt/Agent handler kinds. Codex `hook_runtime.rs`/`hooks.rs`/`hooks_config.rs` + legacy browser-use-core hooks. NEW module `hooks/*` + pub mod line. Network-free tests (fake command runner).
