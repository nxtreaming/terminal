...
### ▶ S-prompts — IN PROGRESS
- [ ] WP-S-prompts-debrand — de-brand codex→browser-use across prompt/template STRINGS + internal identifiers, KEEPING the BROWSER_AGENT preamble + browser-interaction-skills + sync-image notes. May touch SHARED prompt/template files — scope to strings only, serialize alone, re-test hard.
