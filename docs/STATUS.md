...
- [ ] **WP-Safety-3 network-proxy** (IN PROGRESS) — proxy/MITM config + allowlist + deferred network approval. Codex `core/src/network_proxy_loader.rs` (+ `_tests.rs`) → sibling crate `codex_network_proxy` (`network-proxy/`): `build_proxy_allowlist`/`parse_proxy_allowlist_entry`/`host_matches_allowlist_entry`/`normalize_host`, `NetworkProxyConfig`, deferred-approval aligned w/ agent `tools/runtime.rs` `network_policy_decision`. NEW module `network/*`. Allowlist+config+approval REAL; MITM proxy-server thread = honest seam (no fake working MITM). Legacy has no proxy (codex-only).
- [ ] WP-Safety-4 guardian + approval-wiring — LLM-reviewer (fail-closed, circuit breaker) + flip orchestrator `NoneSandboxProvider`→`PlatformSandboxProvider` (real provider now exists from Safety-1) + PermissionRequest precedence (consumes hooks PermissionRequest event + codex ReviewDecision cache). Touches existing orchestrator wiring — scope carefully. LAST autonomous WP before [BLOCKED-NEEDS-HUMAN] cutover.
